...
describe("bezierHorizontal", () => {
  test("直線的な水平パス", () => {
    const result = bezierHorizontal(0, 100, 200, 100);
    expect(result).toBe("M 0 100 C 100 100, 100 100, 200 100");
  });

  test("制御点がX座標の中間になる", () => {
    const result = bezierHorizontal(100, 0, 300, 200);
    expect(result).toBe("M 100 0 C 200 0, 200 200, 300 200");
  });

  test("右から左への逆方向のパス", () => {
    const result = bezierHorizontal(300, 100, 100, 100);
